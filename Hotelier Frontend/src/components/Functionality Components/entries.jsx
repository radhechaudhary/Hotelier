...
function Entries() {
  ...
  const handleChange = (e) => { // 
    ...
    // filtering the filteredData
    setFilteredData(() => { 
      return data.filter((entry) => {
        return entry[searchFor]
          .toString()
          .toUpperCase()
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      id="enteries"
      className="flex-1 bg-[url('/asfalt-dark.png')] md:rounded-tl-2xl flex-grow bg-blue-50 shadow-black shadow-md text-slate-700  ml-[0.5px] h-[calc(100vh-60px)] p-5 md:p-10 flex flex-col gap-7 items-center overflow-hidden"
    >
      <Tooltip
        title="Here you get all the previously stayed customers and their details. click on a row to expand and print details."
      ...
      <div className="flex flex-col gap-1 flex-grow h-full w-full">
        <div className="flex">
          <select
            onChange={TypeChange}
            className="border border-slate-950 w-[70px] rounded-xl rounded-r-none active:outline-none active:border-collapse px-1 py-1 text-[14px]"
          >
          ...
          <input
            ...
            value={inputValue}
            className="border border-slate-950 w-[200px] rounded-xl rounded-l-none active:outline-none active:border-collapse px-2 py-1"
            placeholder="search"
            type={inputType}
          />
        </div>
        <AnimatePresence>
          <div className="overflow-y-scroll pb-10 overflow-x-auto custom-scrollbar">
            <motion.table
              ...
              className=" flex-1 w-full border border-gray-300 text-left "
            >
        ...
        {openedCard ? (
          ...
          >
            <motion.div
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              className="card flex flex-col gap-2 absolute w-[350px] bg-slate-200 rounded-3xl px-4 py-6 shadow-md shadow-black"
            >
              {Object.keys(currentEntry).map((key, index) => {
                return (
                  <p className="font-medium text-slate-700" key={index}>
                    <span className=" font-bold">{headers[index]}</span>:{" "}
                    {currentEntry[key]}
                  </p>
                );
              })}
              <div className="buttons flex w-full gap-[20px] flex-grow">
                <button
                  onClick={() => setOpenedCard(false)}
                  className="bg-blue-500 text-white p-2 flex-1 rounded-[400px] active:bg-blue-600"
                >
                  Ok
                </button>
                <button className="bg-green-500 text-white p-2 flex-1 rounded-[400px] active:bg-green-600">
                  Print
                </button>
              </div>
            </motion.div>
