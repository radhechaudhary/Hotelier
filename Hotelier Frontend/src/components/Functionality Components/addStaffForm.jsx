...
function AddStaffForm(prop) {
  ...
  const submitForm = (e) => {
    ...
    dispatch(
      setStaff([
        ...staff,
        { ...formData, joiningDate: formattedDate, id: staff.length },
      ])
    );
    prop.setAdding(false);
    axios.post( // send the new staff details to backend
      "http://localhost:4000/change-staff",
      {
        staff: [
          ...staff,
          { ...formData, joiningDate: formattedDate, id: staff.length },
    ...
    >
      ...
      >
        <div
          ...
          className="absolute top-3 right-3 w-5 h-5 flex justify-center items-center rounded-full bg-rose-500 text-white p-0 active:bg-rose-700 active:scale-95 cursor-pointer"
        >
          x
        </div>
